package L5.A2;

import java.util.concurrent.locks.Lock;

public class Fir extends Thread {


    private Lock l1;
    private Lock l2;
    private int activity_min1, activity_max1, activity_min2, activity_max2, sleep;

    public Fir(Lock l1, Lock l2, int activity_min1, int activity_max1, int activity_min2, int activity_max2, int sleep) {

        this.l1 = l1;
        this.l2 = l2;
        this.activity_max1 = activity_max1;
        this.activity_min1 = activity_min1;
        this.activity_max2 = activity_max2;
        this.activity_min2 = activity_min2;
        this.sleep = sleep;
    }


    @Override
    public void run() {
        System.out.println(this.getName() + " - STATE 1");
        int k = (int) Math.round(Math.random() * (activity_max1
                - activity_min1) + activity_min1);

        for (int i = 0; i < k * 100000; i++) {
            i++;
            i--;
        }

        l1.lock();

        System.out.println(this.getName() + " - STATE 2");
        int k2 = (int) Math.round(Math.random() * (activity_max2
                - activity_min2) + activity_min2);

        for (int i = 0; i < k2 * 100000; i++) {
            i++;
            i--;
        }
        if (l2.tryLock()) {
            System.out.println(this.getName() + " - STATE 3");
            try {
                Thread.sleep(sleep);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
            l1.unlock();
            l2.unlock();
        } else {
            l1.unlock();
            try {
                sleep(100);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
            l1.lock();
            l2.lock();
            System.out.println(this.getName() + " - STATE 3");
            try {
                Thread.sleep(sleep);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
            l1.unlock();
            l2.unlock();
        }
        System.out.println(this.getName() + " - STATE 4");
    }
}
